...
private:
    ...
    static bool timer_callback(struct repeating_timer *t) {
        auto* self = static_cast<SonicSensor*>(t->user_data);
...
public:
    void init(uint32_t trig, uint32_t echo, pio_hw_t* pio, int sm) {
        hw = new DistanceSensor{pio, static_cast<uint>(sm), static_cast<uint>(trig)};
    ...
    bool changed() {
        ...
        if (std::abs(d - last_reported) > 1) {
            last_reported = d;
            current_val = static_cast<float>(d);
...
inline SonicSensor dist_sensor;


inline void addDistanceSensor(uint32_t trig, uint32_t echo) {
    dist_sensor.init(trig, echo, pio1, 3);
}

inline bool processDistanceSensor() {
